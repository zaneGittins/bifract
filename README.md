...
The easiest way to install Bifract is with the Linux setup wizard. It handles SSL, secure passwords, Docker Compose, and database initialization. This downloads the `bifractctl` binary and runs the interactive setup:
```bash
curl -sfL https://raw.githubusercontent.com/zaneGittins/bifract/main/scripts/install.sh | sh
```

To upgrade an existing installation (if `bifractctl` is already installed):
```bash
sudo bifractctl --upgrade
```

This automatically checks for a newer version of `bifractctl` itself, downloads it if available, then runs the upgrade.
