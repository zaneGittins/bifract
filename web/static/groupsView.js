...
const GroupsView = {
    ...
    async createGroup() {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.success) {
                if (window.Toast) Toast.success('Created', `Group "${name}" created`);
                this.hideCreateForm();
    ...
    async editGroup() {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.success) {
                if (window.Toast) Toast.success('Updated', 'Group updated');
    ...
    async deleteGroup() {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.success) {
                if (window.Toast) Toast.success('Deleted', `Group "${this.selectedGroup.name}" deleted`);
                this.closeDetail();
                await this.loadGroups();
            } else {
    ...
    async addMember() {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.success) {
                if (window.Toast) Toast.success('Added', `${username} added to group`);
                await this.loadMembers();
                await this.loadUsersForMemberSelect();
    ...
    async removeMember(username) {
        ...
        try {
            ...
            const data = await resp.json();
            if (data.success) {
                if (window.Toast) Toast.success('Removed', `${username} removed from group`);
                await this.loadMembers();
                await this.loadUsersForMemberSelect();
                if (this.selectedGroup) {
    ...
    renderPermissions(permissions, fractalId) {
        ...
        if (permissions.length === 0) {
            container.innerHTML = '<div class="no-data" style="padding: 0.75rem; text-align: center; color: var(--text-muted);">No permissions granted. Only tenant admins can access this fractal.</div>';
            return;
        }
        ...
        permissions.forEach(p => {
            ...
            html += `<tr>
                ...
                <td><button class="btn-delete-user btn-sm" onclick="GroupsView.revokePermission('${Utils.escapeJs(p.id)}', '${Utils.escapeJs(fractalId)}')">Revoke</button></td>
            </tr>`;
        });
        html += '</tbody></table>';
        container.innerHTML = html;
