...
const Notebooks = {
    ...
    renderGraphInNotebook(containerId, results) {
        ...
        nodeDetails.forEach((details, id) => {
            ...
            nodes.add({
                ...
                size: 16,
                color: {
                    background: isParent ? (cv('--graph-node-parent') || cv('--accent-primary') || '#3498db') : (cv('--graph-node-child') || cv('--bg-tertiary') || '#555'),
                    border: isParent ? (cv('--graph-node-parent') || cv('--accent-primary') || '#3498db') : (cv('--graph-node-child') || cv('--bg-tertiary') || '#555'),
                },
                font: { color: cv('--graph-label') || cv('--text-primary') || '#eee', size: 11, face: 'Inter', vadjust: -4, strokeWidth: 3, strokeColor: cv('--graph-label-stroke') || 'rgba(0,0,0,0.5)' },
                shape: 'dot',
            });
        });
        ...
        limitedResults.forEach((result) => {
            ...
            if (childId && parentId) {
                edges.add({
                    ...
                    color: { color: cv('--graph-edge') || cv('--border-color') || '#555', opacity: 0.5 },
                    width: 1.5,
    ...
    renderChartOnCanvas(chartId, results) {
        ...
        try {
            if (chartType === 'piechart') {
                ...
                this.renderHistogramChart(canvas, chartData, results);
            }
        } catch (error) {
    ...
    renderSingleValSection(results, sectionConfig) {
        ...
        return `
            <div class="singleval-display" style="border: 1px solid var(--border-color); border-radius: 6px; background: var(--bg-secondary);">
    ...
    renderHistogramChart(canvas, data, results) {
        ...
        const chartConfig = results.chart_config || {};
        const fieldName = chartConfig.field || 'value';
        const cv = ThemeManager.getCSSVar;
    ...
    renderHeatmapInNotebook(containerId, results) {
        ...
        const chartConfig = results.chart_config || {};
        const xField = chartConfig.xField || '_heatmap_x';
        const yField = chartConfig.yField || '_heatmap_y';
        ...
        const availableWidth = container.clientWidth || 700;
        ...
        yVals.forEach((y, j) => {
            xVals.forEach((x, i) => {
                ...
                if (cellW >= 36 && cellH >= 28 && v > 0) {
                    ...
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText(this.formatHeatmapVal(v), cx + cellW / 2, cy + cellH / 2);
                }
            });
        });
        ...
        for (let t = 0; t <= tickCount; t++) {
            ...
            ctx.fillText(this.formatHeatmapVal(val), legendX + legendBarWidth + 6, ty);
        }
            ...
            editorHtml = `
                <div class="inline-editor">
                    <div class="editor-fields">
                        ...
                        <div class="form-group">
                            <label for="edit-content-${section.id}">Quandrix Query</label>
    ...
    updateQuerySyntaxHighlight(inputId, highlightId) {
        ...
        // Use the same highlighting logic as the main search bar (Quandrix mode)
        let highlighted;
