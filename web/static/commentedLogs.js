...
const CommentedLogs = {
    ...
    async show() {
        if (window.CommentGraph) CommentGraph.showListDefault();
    ...
    async bulkAddTag() {
        ...
        try {
            ...
            if (window.Toast) Toast.success('Tags Updated', `Added "${tag.trim()}" to ${count} comment${count !== 1 ? 's' : ''}`);
            this.selectedIds.clear();
            await this.fetchComments();
        } catch (err) {
    ...
    async bulkRemoveTag() {
        ...
        try {
            ...
            const count = data.data?.updated || 0;
            if (window.Toast) Toast.success('Tags Updated', `Removed "${tag.trim()}" from ${count} comment${count !== 1 ? 's' : ''}`);
            this.selectedIds.clear();
            await this.fetchComments();
        } catch (err) {
    ...
    async bulkDelete() {
        ...
        try {
            ...
            if (window.Toast) Toast.success('Comments Deleted', `Deleted ${deleted} comment${deleted !== 1 ? 's' : ''}`);
            this.selectedIds.clear();
            await this.fetchComments();
    ...
    async showGenerateNotebookModal() {
        ...
        const attackChainOption = aiEnabled ? `
            <div style="padding: 0 1.25rem 0.5rem;">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer; color: var(--text-primary); font-size: 0.9rem;">
                    <input type="checkbox" id="generateNotebookAttackChain"
                           style="accent-color: var(--accent-primary); width: 16px; height: 16px; cursor: pointer;">
                    AI Attack Chain Summary
                </label>
                <small style="color: var(--text-muted); display: block; margin-top: 0.25rem; padding-left: 1.5rem;">
        ...
        const tagPills = tags.map(t => {
            ...
            return `<button type="button" class="generate-nb-tag" data-tag="${escaped}">${escaped}</button>`;
        }).join('');
                ...
                <div class="modal-content" style="max-width: 420px;">
                    ...
                    <form id="generateNotebookForm">
                        ...
                        <div class="form-group" style="padding: 1rem 1.25rem;">
                            <label>Tag</label>
                            <div id="generateNotebookTagList" style="display: flex; flex-wrap: wrap; gap: 0.375rem; margin-top: 0.375rem;">
                                ${tagPills}
                            </div>
                        ...
                        <div class="modal-buttons" style="padding: 0.75rem 1.25rem 1.25rem;">
                            <button type="button" class="btn-secondary" onclick="CommentedLogs.closeGenerateNotebookModal()">Cancel</button>
                            <button type="submit" class="btn-primary" id="generateNotebookSubmitBtn" disabled>Generate</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        document.getElementById('generateNotebookTagList').addEventListener('click', (e) => {
            const pill = e.target.closest('.generate-nb-tag');
            if (!pill) return;
            document.querySelectorAll('.generate-nb-tag').forEach(p => p.classList.remove('selected'));
            pill.classList.add('selected');
            document.getElementById('generateNotebookTag').value = pill.dataset.tag;
