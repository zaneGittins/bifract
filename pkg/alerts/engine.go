...
func (e *Engine) refreshAlertsCache(ctx context.Context) ([]*Alert, error) {
	query := `
		...
		GROUP BY a.id, a.name, a.description, a.query_string, a.alert_type, a.enabled,
		         a.throttle_time_seconds, a.throttle_field, a.labels, a.fractal_id, a.prism_id,
		         a.created_by, a.created_at, a.updated_at, a.last_triggered,
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			pq.Array(&alert.Labels), &alert.FractalID, &alert.PrismID,
...
func (e *Engine) recordExecution(ctx context.Context, alertID string, fractalID string, logCount int, throttled bool, throttleKey string, executionTimeMs int, webhookResults []WebhookResult, fractalResults []FractalResult) error {
	...
	_, err = e.pg.Exec(ctx,
		`INSERT INTO alert_executions (alert_id, fractal_id, log_count, throttled, throttle_key, execution_time_ms, webhook_results, fractal_results)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
		alertID, fractalID, logCount, throttled, throttleKey, executionTimeMs, string(webhookResultsJSON), string(fractalResultsJSON),
	)
	if err != nil {
		var pgErr *pq.Error
