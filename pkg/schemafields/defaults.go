...
var ProjectDefaultFields = []SchemaField{
	...
	// Cross-tree reconnection fields (pgr). The provenance graph reads these to
	// link activity across separate process trees: file_write -> execute via
	// target_file, and process_access / remote_thread via target_image. Hint-only
	// for the same reason as parent_process_guid: pgr reaches them through
	// process_edges and proc_lineage, which carry their own indexes, so a bloom
	// here would tax every insert to prune a query never issued against logs
	// directly. The source/target GUID pair is deliberately not hinted: it is
	// resolved entirely through those edge tables.
	//
	// These are required by a Bifract feature rather than assumed about the
	// user's data, which is the bar for shipping a default. A hint that never
	// matches costs nothing: it adds no mutation, no storage, and (unlike a
