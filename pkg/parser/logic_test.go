...
// Bare strings/regex always search norm_log regardless of surrounding conditions.
// Case-insensitive norm_log searches route through lower(norm_log) so the n-gram
// index can prune granules; field-qualified regex is left as a plain (?i) match.
var termTypes = []termDef{
	{"quoted_string", `"error"`, "match(lower(norm_log), 'error')"},
...
func TestPipelineBooleanConditions(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantSQL: []string{"match(lower(norm_log), 'error')", "match(lower(norm_log), 'warning')", " OR "},
		},
		{
			name:    "string AND in pipeline",
			query:   `* | "error" AND "warning"`,
			wantSQL: []string{"match(lower(norm_log), 'error')", "match(lower(norm_log), 'warning')"},
		},
		{
			name:    "NOT regex in pipeline",
			query:   `* | NOT /error/`,
			wantSQL: []string{"NOT match(norm_log,"},
		},
		{
			name:    "NOT string in pipeline",
			query:   `* | NOT "error"`,
			wantSQL: []string{"NOT match(lower(norm_log),"},
		},
		{
		...
		},
		{
			...
			wantSQL: []string{"match(lower(norm_log), 'error')", "fields.`status`::String = '500'", " OR "},
		},
		{
			name:    "field AND string in pipeline",
			query:   `* | status="200" AND "success"`,
			wantSQL: []string{"fields.`status`::String = '200'", "match(lower(norm_log), 'success')"},
		},
		{
		...
		},
		{
			...
			wantSQL: []string{"NOT match(lower(norm_log),", " OR ", "match(lower(norm_log), 'warning')"},
		},
		{
		...
		},
		{
			...
			wantSQL: []string{"match(lower(norm_log), 'error')", "NOT match(lower(norm_log),", " OR "},
		},
		{
		...
		},
		{
			...
			wantSQL: []string{"match(lower(norm_log), 'error')", "match(lower(norm_log), 'warning')", "fields.`service`::String = 'web'"},
		},
		{
			name:    "regex OR field in pipeline",
			query:   `* | /warn/i OR level="error"`,
			wantSQL: []string{"match(lower(norm_log), 'warn')", "fields.`level`::String = 'error'", " OR "},
		},
		{
		...
		},
		{
			...
			wantSQL: []string{"match(lower(norm_log), 'error')", "match(lower(norm_log), 'warning')", "fields.`status`::String = '500'"},
		},
...
func TestOperatorPrecedence(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			query:   `"A" OR "B" AND "C"`,
			wantSQL: []string{"match(lower(norm_log), 'a')", "match(lower(norm_log), 'b')", "match(lower(norm_log), 'c')", " OR "},
		},
		{
			name:    "mixed: string OR field AND field",
			query:   `"A" OR status="200" AND service="web"`,
			wantSQL: []string{"match(lower(norm_log), 'a')", "fields.`status`::String = '200'", "fields.`service`::String = 'web'", " OR "},
		},
		{
