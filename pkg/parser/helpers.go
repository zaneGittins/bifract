...
// convertTimeFormat converts Quandrix time format to ClickHouse format
func convertTimeFormat(quandrixFormat string) string {
	// Convert common format patterns to ClickHouse
	format := quandrixFormat
...
func processStatsFn(fn string, selectFields *[]string, computedFields map[string]bool) bool {
	fn = strings.TrimSpace(fn)

	if fn == "count()" || strings.HasPrefix(fn, "count(") {
		params := parseStatsFunctionParams(fn, "count")
		field := params["field"]
		if field == "" {
		...
		if field != "" && distinct {
			...
			*selectFields = append(*selectFields, fmt.Sprintf("uniqExact(%s) AS %s", jsonFieldRef(field), alias))
			computedFields[alias] = true
		} else if field != "" {
			if alias == "" {
				alias = "total"
			}
			*selectFields = append(*selectFields, fmt.Sprintf("count(%s) AS %s", jsonFieldRef(field), alias))
			computedFields[alias] = true
		} else {
			if alias == "" {
				alias = "_count"
		...
		return true
	} else if strings.HasPrefix(fn, "avg(") {
		field := extractFunctionField(fn, "avg")
		*selectFields = append(*selectFields, fmt.Sprintf("avg(toFloat64OrNull(%s)) AS _avg", jsonFieldRef(field)))
		return true
	} else if strings.HasPrefix(fn, "sum(") {
		field := extractFunctionField(fn, "sum")
		*selectFields = append(*selectFields, fmt.Sprintf("sum(toFloat64OrNull(%s)) AS _sum", jsonFieldRef(field)))
		return true
	} else if strings.HasPrefix(fn, "max(") {
		field := extractFunctionField(fn, "max")
		if field == "timestamp" {
			*selectFields = append(*selectFields, "max(timestamp) AS max_timestamp")
		} else {
			*selectFields = append(*selectFields, fmt.Sprintf("max(toFloat64OrNull(%s)) AS _max", jsonFieldRef(field)))
		}
		return true
	} else if strings.HasPrefix(fn, "min(") {
		field := extractFunctionField(fn, "min")
		if field == "timestamp" {
			*selectFields = append(*selectFields, "min(timestamp) AS min_timestamp")
		} else {
			*selectFields = append(*selectFields, fmt.Sprintf("min(toFloat64OrNull(%s)) AS _min", jsonFieldRef(field)))
		}
		return true
	} else if strings.HasPrefix(fn, "percentile(") {
		field := extractFunctionField(fn, "percentile")
		*selectFields = append(*selectFields, fmt.Sprintf("quantiles(0.5, 0.75, 0.99)(toFloat64OrNull(%s)) AS percentile_%s", jsonFieldRef(field), escapeString(field)))
		return true
	} else if strings.HasPrefix(fn, "stdDev(") || strings.HasPrefix(fn, "stddev(") {
		var field string
		if strings.HasPrefix(fn, "stdDev(") {
			field = extractFunctionField(fn, "stdDev")
		} else {
			field = extractFunctionField(fn, "stddev")
		}
		*selectFields = append(*selectFields, fmt.Sprintf("stddevPop(toFloat64OrNull(%s)) AS stddev_%s", jsonFieldRef(field), escapeString(field)))
		return true
	} else if strings.HasPrefix(fn, "skewness(") || strings.HasPrefix(fn, "skew(") {
		var field string
		if strings.HasPrefix(fn, "skewness(") {
			field = extractFunctionField(fn, "skewness")
		} else {
			field = extractFunctionField(fn, "skew")
		}
		*selectFields = append(*selectFields, fmt.Sprintf("skewPop(toFloat64OrNull(%s)) AS skewness_%s", jsonFieldRef(field), escapeString(field)))
		return true
	} else if strings.HasPrefix(fn, "kurtosis(") || strings.HasPrefix(fn, "kurt(") {
		var field string
		if strings.HasPrefix(fn, "kurtosis(") {
			field = extractFunctionField(fn, "kurtosis")
		} else {
			field = extractFunctionField(fn, "kurt")
		}
		*selectFields = append(*selectFields, fmt.Sprintf("kurtPop(toFloat64OrNull(%s)) AS kurtosis_%s", jsonFieldRef(field), escapeString(field)))
		return true
	} else if strings.HasPrefix(fn, "iqr(") {
		field := extractFunctionField(fn, "iqr")
		ref := jsonFieldRef(field)
		*selectFields = append(*selectFields,
			fmt.Sprintf("quantile(0.25)(toFloat64OrNull(%s)) AS iqr_q1_%s", ref, escapeString(field)),
			fmt.Sprintf("quantile(0.75)(toFloat64OrNull(%s)) AS iqr_q3_%s", ref, escapeString(field)),
			fmt.Sprintf("quantile(0.75)(toFloat64OrNull(%s)) - quantile(0.25)(toFloat64OrNull(%s)) AS iqr_%s", ref, ref, escapeString(field)))
		...
		return true
	} else if strings.HasPrefix(fn, "selectfirst(") {
		field := extractFunctionField(fn, "selectfirst")
		if field == "timestamp" {
			*selectFields = append(*selectFields, "min(timestamp) AS first_timestamp")
		} else {
			*selectFields = append(*selectFields, fmt.Sprintf("argMin(%s, timestamp) AS first_%s", jsonFieldRef(field), escapeString(field)))
		}
		return true
	} else if strings.HasPrefix(fn, "selectlast(") {
		field := extractFunctionField(fn, "selectlast")
		if field == "timestamp" {
			*selectFields = append(*selectFields, "max(timestamp) AS last_timestamp")
		} else {
			*selectFields = append(*selectFields, fmt.Sprintf("argMax(%s, timestamp) AS last_%s", jsonFieldRef(field), escapeString(field)))
		}
		return true
	} else if strings.HasPrefix(fn, "collect(") {
		field := extractFunctionField(fn, "collect")
		...
		*selectFields = append(*selectFields, fmt.Sprintf("groupArray(%s) AS %s", fieldRef, alias))
		computedFields[alias] = true
		return true
	} else if strings.HasPrefix(fn, "top(") {
		params := parseStatsFunctionParams(fn, "top")
		field := params["field"]
		if field == "" {
			field = params["_positional"]
		}
		...
		computedFields[alias] = true
		return true
	} else if strings.HasPrefix(fn, "median(") {
		field := extractFunctionField(fn, "median")
		*selectFields = append(*selectFields, fmt.Sprintf("median(toFloat64OrNull(%s)) AS median_%s", jsonFieldRef(field), escapeString(field)))
		return true
	} else if strings.HasPrefix(fn, "mad(") {
		field := extractFunctionField(fn, "mad")
		fieldRef := jsonFieldRef(field)
		*selectFields = append(*selectFields, fmt.Sprintf("arrayReduce('median', arrayMap(x -> abs(x - arrayReduce('median', groupArray(toFloat64OrNull(%s)))), groupArray(toFloat64OrNull(%s)))) AS mad_%s", fieldRef, fieldRef, escapeString(field)))
		return true
	}
	return false
}
