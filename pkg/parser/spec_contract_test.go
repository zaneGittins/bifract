...
var commandProbes = map[string]commandProbe{
	...
	"modelLookup": {base: `* | modelLookup(model="m", key=[user])`, with: map[string]string{"model": `* | modelLookup(model="n", key=[user])`, "key": `* | modelLookup(model="m", key=[user,host])`, "strict": `* | modelLookup(model="m", key=[user], strict=false)`}},
