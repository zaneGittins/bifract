...
// SelfUpdate checks for a newer bifract binary and re-execs if one is found.
...
func SelfUpdate(origArgs []string) {
	...
	printStep("Checking for bifract updates...")
	...
	if CompareVersions(Version, latest) >= 0 {
		printDone(fmt.Sprintf("bifract is up to date (%s)", Version))
		return
	}

	fmt.Printf("[%s] %s %s -> %s\n",
		SuccessStyle.Render("+"),
		PromptStyle.Render("New bifract available:"),
		DimStyle.Render(Version),
		ValueStyle.Render(latest),
	)

	fmt.Printf("    Update bifract before upgrading? [Y/n] ")
	reader := bufio.NewReader(os.Stdin)
	answer, _ := reader.ReadString('\n')
	answer = strings.TrimSpace(strings.ToLower(answer))
	if answer != "" && answer != "y" && answer != "yes" {
		printDone(fmt.Sprintf("Continuing with bifract %s", Version))
		return
	}

	binaryURL := fmt.Sprintf(
		"https://github.com/zaneGittins/bifract/releases/download/%s/bifract-%s-%s",
		latest, runtime.GOOS, runtime.GOARCH,
	)

	printStep("Downloading bifract " + latest + "...")
	...
	printDone("Updated bifract to " + latest)
	fmt.Println()
