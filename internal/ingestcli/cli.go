...
func PrintIngestUsage() {
	title := TitleStyle.Render("bifractctl --ingest")

	fmt.Fprintf(os.Stderr, `%s - Bulk log ingestion for Bifract

%s
  bifractctl --ingest <files...> --token <token> [options]
...
%s
  ...
  When --workers and --batch-size are omitted, bifractctl runs in auto
...
%s
  bifractctl --ingest logs.json --token bifract_ingest_abc123
  bifractctl --ingest *.json --token bifract_ingest_abc123
  bifractctl --ingest access.csv --token bifract_ingest_abc123 --workers 8 --batch-size 2000
  bifractctl --ingest -r "*.json" --token bifract_ingest_abc123
  bifractctl --ingest logs.json --token bifract_ingest_abc123 --url https://bifract.local --insecure
`,
		title,
