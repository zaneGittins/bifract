...
Dictionaries are per-fractal lookup tables for enriching log data at query time. Define a dictionary with key-value columns, then use `match()` in Quandrix queries to join dictionary data onto your results.
...
Use `match()` to enrich query results with dictionary data. See [Enrichment](../quandrix/enrichment.md) for full syntax.
