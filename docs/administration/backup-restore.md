...
System-level backups are managed through the `bifract` CLI. These back up the entire PostgreSQL database including users, fractal configurations, alerts, saved searches, notebooks, and all metadata.
...
```bash
bifract --backup --dir /opt/bifract --non-interactive
```

### Listing Backups

```bash
bifract --list-backups --dir /opt/bifract
```
