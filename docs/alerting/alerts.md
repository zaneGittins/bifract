...
Alerts run Quandrix queries on a schedule and trigger actions on hits. A background ticker (default 30 seconds, `BIFRACT_ALERT_EVAL_INTERVAL`) evaluates all enabled alerts using a cursor-based approach on the ingest timestamp. Each alert tracks `last_evaluated_at`, so no logs are missed across restarts.
