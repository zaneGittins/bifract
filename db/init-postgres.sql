...
-- bulk toggles and engine auto-disable cannot evict real edits.
...
-- Sample events an alert should or should not match, run from the alert editor.
-- Events are stored normalized, so a test needs no normalizer and survives
-- normalizer changes. Not part of revision content: tests describe the definition
...
-- Pages may belong to a folder; NULL means the library root. ON DELETE SET NULL so
-- deleting a folder drops its pages back to the root rather than destroying them.
...
-- Every rule a feed's repository offers, whether or not it became an alert. The
-- alerts table only records what was imported, so it cannot answer "what exists
-- for this ATT&CK technique that we are not running, and why not".
...
-- Exactly one scope column must be set per row
...
-- Replace global unique name constraints with per-scope unique indexes
...
-- Persisted results of the background schema sweep. The schema tab reads only
-- these tables, so the page renders in Postgres time no matter how large logs is.
...
-- column-budget figures that only ClickHouse part metadata can answer.
...
-- Alert review subsystem. These four tables carry foreign keys to fractals and
-- prisms, so they must be created after both. The app applies this file as a
-- single multi-statement query, which Postgres runs in one implicit transaction:
-- a forward reference here does not degrade, it rolls back the entire schema and
-- leaves a fresh install with no tables at all.
-- Policy checks on an alert definition, scoped like the alerts they judge. A rule is
-- field/operator/value rather than a script, so a CI-style gate needs no language to
-- author or parse. Feed-managed alerts are exempt: their definitions come from upstream.
...
-- Review gate: proposals held apart from the alerts they change, scoped like them.
